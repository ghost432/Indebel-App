...
const Profile = () => {
  ...
  const [proFormData, setProFormData] = useState({
    ...
    reseaux_sociaux: user?.reseau_sociaux || {
      linkedin: '',
      twitter: '',
      facebook: '',
      instagram: ''
  ...
  useEffect(() => {
    ...
    if (user?.id) {
      const savedImage = profileService.getProfileImage(user)
      if (savedImage) setProfileImage(savedImage)
      
      const savedCover = profileService.getCoverImage(user)
  ...
  const handleProSubmit = async (e) => {
    ...
    try {
      ...
      if (dataToSend.competences_recherchees) {
      ...
      if (dataToSend.tarif_journalier) {
        dataToSend.tarif_journalier = parseFloat(dataToSend.tarif_journalier) || 0
      }
      if (dataToSend.experience) {
        dataToSend.experience = parseInt(dataToSend.experience, 10) || 0
      }
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        profileService.saveProfileImage(imageData, user?.id)
        // Synchroniser avec le backend
        try {
          await userService.updateUser(user.id, { photo_profil: imageData })
  ...
  const handleCoverChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        profileService.saveCoverImage(imageData, user?.id)
        // Synchroniser avec le backend
        try {
          await userService.updateUser(user.id, { image_couverture: imageData })
              ...
              <div className="flex flex-col items-center space-y-2 mb-2">
                <div className="text-center mt-2">
                  <h2 className="text-2xl font-bold text-slate-900 mt-2">
                    {user?.role === 'employer' 
                      ? (user?.denomination || 'Mon Recruteur')
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Le numéro d'entreprise (BCE) de votre société
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <MapboxAutocomplete
                      ...
                      onChange={(value) => setProFormData({...proFormData, adresse: value})}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      disabled={!editingPro}
                      className="w-full min-h-[120px] px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                      style={{
                  ...
                  {user?.langues_parlees && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {(Array.isArray(user.langues_parlees) ? user.langues_parlees : JSON.parse(user.langues_parlees || '[]')).map((langue, index) => (
