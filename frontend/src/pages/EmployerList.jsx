...
const EmployerList = () => {
  ...
  const [isUnlocked, setIsUnlocked] = useState(() => {
    ...
    return sessionStorage.getItem(`unlocked_employer_list_${user?.id}`) === 'true';
  })
  ...
  const fetchCreditInfo = async () => {
    try {
      ...
      if (settingsRes.data?.cout_vues_employers !== undefined) {
        setCreditCost(parseInt(settingsRes.data.cout_vues_employers, 10));
      }
  ...
  const handleConfirmUnlock = async () => {
    ...
    setUnlocking(true);
    try {
      const res = await api.post('/credits/consume', { action: 'view_employers_list' });
      if (res.data?.success) {
        sessionStorage.setItem(`unlocked_employer_list_${user?.id}`, 'true');
        setIsUnlocked(true);
        setShowConfirmUnlock(false);
    ...
    <div className="relative min-h-[60vh]">
      ...
      <div className={!isUnlocked ? "filter blur-md pointer-events-none select-none transition-all duration-300" : ""}>
        <section className="mb-8 rounded-[28px] bg-white p-8 shadow-sm ring-1 ring-slate-200">
          ...
          {/* Credit Banner */}
          <div className="mt-6 p-4 rounded-2xl bg-indigo-50/80 border border-indigo-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-xl bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold">
                💳
              </div>
              <div>
                <p className="text-sm font-bold text-indigo-950">Accès et consultation de l'annuaire recruteurs</p>
                <p className="text-xs text-indigo-800">
                  Coût défini par l'administration : <span className="font-bold text-indigo-900">{creditCost} crédit(s)</span> par consultation.
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-slate-700 bg-white px-3 py-1.5 rounded-full border border-indigo-200 shadow-sm">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-slideUp border border-slate-100">
            <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-indigo-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900">Consulter l'annuaire des entreprises</h3>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-slideUp border border-red-100">
            ...
            <div className="p-6 space-y-5 text-center">
              <p className="text-sm text-slate-700 leading-relaxed">
                Votre solde actuel est de <strong className="text-red-600 font-bold">{creditBalance} crédit(s)</strong>. Vous avez besoin de <strong className="text-slate-900 font-bold">{creditCost} crédit(s)</strong> pour consulter la liste des entreprises.
              </p>
