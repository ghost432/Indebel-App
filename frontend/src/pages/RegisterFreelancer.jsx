...
const RegisterFreelancer = () => {
  ...
  useEffect(() => {
    ...
    document.title = `${stepTitles[step]} - Inscription Prestataire - Indebel`
  }, [step])
    ...
    if (step === 1) {
      ...
      if (manualMode && !bceVerified) {
        setFormData({
          ...formData,
          numero_bce: bceNumber,
          denomination: formData.denomination || 'Prestataire',
        ...
        <div className="flex items-center justify-between">
          {steps.map((label, index) => (
            <div key={index} className="flex flex-col items-center flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium ${step > index + 1 ? 'bg-green-600 text-white' :
              ...
              {index < steps.length - 1 && (
                <div className={`absolute w-full h-1 ${step > index + 1 ? 'bg-green-600' : 'bg-gray-200'}`}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-green-50 to-emerald-50 relative">
      {/* Retour au site */}
      <a
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    className="w-32 px-2 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500"
                  >
                    {pays.map(p => (
