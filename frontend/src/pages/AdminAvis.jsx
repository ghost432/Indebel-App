...
const AdminAvis = () => {
  ...
  const filteredAvis = avis.filter(item => {
    ...
    return (
      (item.nom_auteur || '').toLowerCase().includes(s) ||
  ...
  const load = async () => {
    try {
      setLoading(true)
      const res = await avisService.getAdminAvis({ limit: 1000 })
  ...
  useEffect(() => {
    document.title = 'Avis prestataires - Indebel'
    load()
  }, [])
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault()
    try {
      await avisService.updateAdminAvis(editingAvis.id, {
        nom_auteur: editingAvis.nom_auteur,
  ...
  const remove = async (id) => {
    ...
    try {
      await avisService.deleteAdminAvis(id)
  ...
  return (
    <div className="space-y-7">
      <section className="rounded-[28px] bg-white border border-slate-100 p-7 text-slate-800 shadow-sm">
        <p className="text-xs font-black uppercase tracking-[0.24em] text-white/55">Réputation publique</p>
        <h1 className="mt-3 text-3xl font-black">Avis des particuliers</h1>
        <p className="mt-2 max-w-2xl text-white/70">Modérez les avis reçus depuis la page publique et rattachez-les au bon prestataire.</p>
      </section>
            ...
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  <h2 className="text-lg font-black text-[#082151]">{item.nom_auteur}</h2>
                  <span className="inline-flex items-center gap-1 font-bold text-[#c02525]"><Star className="h-4 w-4 fill-current" /> {item.note}/5</span>
                </div>
                <p className="mt-1 text-sm text-slate-500">Prestataire: {item.freelancer_prenom} {item.freelancer_nom}</p>
              </div>
        ...
        {editingAvis && (
          <form onSubmit={handleEditSubmit} className="space-y-4">
            <div>
              ...
              <input type="text" className="input w-full" value={editingAvis.nom_auteur} onChange={(e) => setEditingAvis({ ...editingAvis, nom_auteur: e.target.value })} required />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Note (/5)</label>
