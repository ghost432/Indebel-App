  ...
  return (
    <div className="container-custom py-8">
      <section className="mb-10 rounded-[28px] bg-white p-8 shadow-sm ring-1 ring-slate-200">
        ...
        <h1 className="mt-3 text-3xl md:text-4xl font-black text-[#082151]">Nos forfaits recruteurs</h1>
            ...
            <div className="flex-1">
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="mt-4 pt-3 border-t border-blue-200 grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-gray-600">
                  <span>💼 Missions&nbsp;: <strong>{forfaitActuel.max_missions ?? 'Illimité'}</strong></span>
                  <span>📝 Documents&nbsp;: <strong>{forfaitActuel.max_documents ?? 'Illimité'}</strong></span>
                  {forfaitActuel.max_candidatures_recues != null && <span>📥 Candidatures reçues/mois&nbsp;: <strong>{forfaitActuel.max_candidatures_recues ?? 'Illimité'}</strong></span>}
                  {forfaitActuel.max_demandes_devis != null && <span>✉️ Demandes devis/mois&nbsp;: <strong>{forfaitActuel.max_demandes_devis ?? 'Illimité'}</strong></span>}
                  {forfaitActuel.max_devis_recus != null && <span>📬 Devis reçus/mois&nbsp;: <strong>{forfaitActuel.max_devis_recus ?? 'Illimité'}</strong></span>}
                </div>
              </div>
            </div>
          ...
          return (
            ...
            >
              ...
              <div className="space-y-3 mb-10 flex-1">
                ...
                {/* Missions */}
                <ForfaitFeatureRow label={`${forfait.max_missions != null ? forfait.max_missions + ' mission(s) publiable(s)' : 'Missions illimitées'}`} />

                {/* Candidatures reçues */}
                {forfait.max_candidatures_recues != null ? (
                  <ForfaitFeatureRow label={`${forfait.max_candidatures_recues} candidature(s) reçue(s)/mois`} />
                ) : (
                  <ForfaitFeatureRow label="Candidatures reçues illimitées" />
                )}

                {/* Demandes devis */}
                {forfait.max_demandes_devis != null ? (
                  <ForfaitFeatureRow label={`${forfait.max_demandes_devis} demande(s) de devis/mois`} />
                ) : (
                  <ForfaitFeatureRow label="Demandes de devis illimitées" />
                )}

                {/* Devis reçus */}
                {forfait.max_devis_recus != null ? (
                  <ForfaitFeatureRow label={`${forfait.max_devis_recus} devis reçu(s)/mois`} />
                ) : (
                  <ForfaitFeatureRow label="Devis reçus illimités" />
                )}

                {/* Badge premium */}
                {forfait.badge_premium ? <ForfaitFeatureRow label="Badge entreprise premium" /> : null}

                {/* Mise en avant */}
                {forfait.mise_en_avant ? <ForfaitFeatureRow label="Missions mises en avant" /> : null}
