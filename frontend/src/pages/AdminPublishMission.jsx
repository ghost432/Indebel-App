...
const AdminPublishMission = () => {
  ...
  const fetchEmployers = async () => {
    try {
      ...
      const employersList = (response.data.data || []).filter(user => user.role === 'employer')
  ...
  const handleSelectMissionType = (type) => {
    if (!selectedEmployer) {
      return toast.error('Veuillez d\'abord sélectionner une recruteur')
    }
  ...
  return (
    <div className="py-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-gray-600 mb-8">Sélectionnez une recruteur et le type de mission</p>

        {/* Sélection recruteur */}
        <Card className="mb-6">
          <h3 className="text-lg font-semibold mb-4">1. Sélectionnez le recruteur</h3>
          <select
            value={selectedEmployer}
            onChange={(e) => setSelectedEmployer(e.target.value)}
            className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500"
          >
            <option value="">-- Choisir une recruteur --</option>
            {employers.map(emp => (
          ...
          {employers.length === 0 && (
            <p className="text-sm text-gray-500 mt-2">Aucune recruteur trouvée</p>
          )}
        </Card>
