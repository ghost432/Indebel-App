...
const FreelancerForfaits = () => {
  ...
  const getFeaturesForDisplay = (forfait) => {
    ...
    if (isFeatureActive(forfait.label_indebel)) {
      features.push({
        id: 'label',
        text: 'Label Indebel inclus',
        active: true
      })
    }

    // Accès prioritaire
    if (isFeatureActive(forfait.acces_prioritaire)) {
      features.push({
        id: 'prioritaire',
        text: 'Accès prioritaire',
        active: true
      })
    }

    // Support dédié
    if (isFeatureActive(forfait.support_dedie)) {
      features.push({
        id: 'support',
        text: 'Support dédié',
        active: true
    ...
    // Missions gratuites
    if (forfait.missions_gratuites && forfait.missions_gratuites > 0) {
      features.push({
        id: 'missions_gratuites',
        text: `${forfait.missions_gratuites} missions gratuites`,
        active: true
      })
    }
