  ...
  const handleCoverChange = (e) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        // Sauvegarder dans localStorage
        localStorage.setItem(`coverImage_${user?.id}`, imageData)
        // Synchroniser avec le backend
        try {
              ...
              <div className="relative inline-block">
                <div className="h-32 w-32 bg-gradient-to-br from-primary-500 to-primary-700 rounded-full flex items-center justify-center text-white text-4xl font-bold mx-auto mb-4 overflow-hidden">
                  {profileImage ? (
                    ...
                    {user?.role === 'employer' 
                      ? (user?.denomination?.charAt(0)?.toUpperCase() || 'E')
                      : `${user?.prenom?.charAt(0) || ''}${user?.nom?.charAt(0) || ''}`.toUpperCase() || 'I'}
                  </span>
                  )}
                </div>
                <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      Le numéro d'recruteur (BCE) de votre société
                    </p>
                  </div>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Langues parlées dans le recruteur
                    </label>
                    <select
                      multiple
