...
const AdminTypeFacturation = () => {
  const [types, setTypes] = useState([
    { id: 1, nom: 'Par jour', description: 'Facturation journalière', actif: 1 },
    { id: 2, nom: 'Par semaine', description: 'Facturation hebdomadaire', actif: 1 },
    { id: 3, nom: 'Par mois', description: 'Facturation mensuelle', actif: 1 }
  ])
  const [modal, setModal] = useState({ open: false, data: null })

  useEffect(() => {
    document.title = 'Types de Facturation - Admin'
  }, [])
  ...
  const handleSave = (e) => {
    ...
    if (modal.data?.id) {
      ...
      toast.success('Type de facturation modifié')
    } else {
      setTypes([...types, data])
      toast.success('Type de facturation créé')
    }
    setModal({ open: false, data: null })
  }

  const handleDelete = (id) => {
    if (!confirm('Supprimer ce type de facturation ?')) return
    setTypes(types.filter(t => t.id !== id))
    toast.success('Type de facturation supprimé')
  }
  ...
  return (
    <div className="py-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-[#082151]">Types de Facturation</h1>
          <p className="text-slate-500 mt-1">Gérez les modalités de paiement des missions</p>
        </div>
      ...
      {types.length === 0 ? (
        <Card className="text-center py-16 border-dashed">
          ...
          <h3 className="text-lg font-bold text-[#082151] mb-2">Aucun type trouvé</h3>
          <p className="text-slate-500">Commencez par ajouter un type de facturation.</p>
        </Card>
      ) : (
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span>{modal.data ? 'Modifier le type' : 'Nouveau type de facturation'}</span>
          </div>
        }
      >
...
export default AdminTypeFacturation
