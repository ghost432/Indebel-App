...
const EmployerViewFreelancer = () => {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      const rolePath = user?.role === 'freelancer' ? 'freelancer' : 'employer';
      ...
      if (existingConversation) {
        ...
        navigate(`/${rolePath}/mes-messages?conversation_id=${existingConversation.id}`);
      } else {
        ...
        navigate(`/${rolePath}/mes-messages?conversation_id=${conversationId}`);
      }
    } catch (error) {
      ...
      const rolePath = user?.role === 'freelancer' ? 'freelancer' : 'employer';
      navigate(`/${rolePath}/mes-messages`);
    } finally {
      ...
      <div className="max-w-4xl mx-auto p-6">
        <Card>
          <div className="text-center py-12">
            ...
            <Button onClick={() => {
              const rolePath = user?.role === 'freelancer' ? 'freelancer' : 'employer';
              navigate(`/${rolePath}/list-freelancers`);
            }}>
              Retour à la liste
            </Button>
          </div>
          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-3">
                ...
                <LabelBadge userId={profile.id} size="lg" />
                <VerificationBadge
