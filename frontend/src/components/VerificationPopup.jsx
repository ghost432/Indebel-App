...
const VerificationPopup = ({ isOpen: controlledIsOpen, onClose: controlledOnClose }) => {
  const { user } = useAuth()
  const navigate = useNavigate()
  const [isVisible, setIsVisible] = useState(false)

  // Use controlled state if provided, otherwise fallback to internal state
  const isActuallyVisible = controlledIsOpen !== undefined ? controlledIsOpen : isVisible;

  useEffect(() => {
    // Only auto-show if not controlled
    if (controlledIsOpen === undefined && user && user.role === 'freelancer' && user.statut_verification === 'non_verifie') {
        ...
        >
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-2">
              <p className="text-sm font-bold text-green-950 flex items-center">
                <CheckCircle className="h-4 w-4 mr-2" />
                Avantages de la vérification :
              </p>
              <ul className="text-sm text-green-900 space-y-1 ml-6 font-semibold">
