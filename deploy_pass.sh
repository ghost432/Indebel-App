...
case $choice in
    ...
    4)
        ...
        sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} "cd ${BACKEND_PATH} && pm2 restart indebel-api"
        print_success "API redémarrée"
        sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} "pm2 status"
        exit 0
        ;;
    5)
        print_header "LOGS PM2"
        sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} "pm2 logs indebel-api"
        exit 0
        ;;
    6)
...
if [ "$DEPLOY_FRONTEND" = true ]; then
    ...
    if command -v rsync &> /dev/null; then
        ...
        sshpass -p "BelgiqueDreambis@272829" rsync -avz --delete -e "ssh -o StrictHostKeyChecking=no" frontend/dist/ ${SSH_USER}@${SERVER}:${FRONTEND_PATH}/
    else
        print_info "Utilisation de scp"
        sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} "rm -rf ${FRONTEND_PATH}/*"
        sshpass -p "BelgiqueDreambis@272829" scp -o StrictHostKeyChecking=no -r frontend/dist/* ${SSH_USER}@${SERVER}:${FRONTEND_PATH}/
    fi
    ...
    sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} << 'EOF'
...
if [ "$DEPLOY_BACKEND" = true ]; then
    ...
    sshpass -p "BelgiqueDreambis@272829" scp -o StrictHostKeyChecking=no backend.tar.gz ${SSH_USER}@${SERVER}:/tmp/
...
EOF
    ...
    sshpass -p "BelgiqueDreambis@272829" scp -o StrictHostKeyChecking=no backend/.env.production ${SSH_USER}@${SERVER}:${BACKEND_PATH}/.env
...
echo "  - Voir logs: sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} 'pm2 logs indebel-api'"
echo "  - Redémarrer: sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} 'pm2 restart indebel-api'"
echo "  - Status: sshpass -p "BelgiqueDreambis@272829" ssh -o StrictHostKeyChecking=no ${SSH_USER}@${SERVER} 'pm2 status'"
echo ""
