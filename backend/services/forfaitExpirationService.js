...
class ForfaitExpirationService {
  ...
  async checkExpiringForfaits() {
    try {
      ...
      // -- NOUVEAU: Rétrograder les utilisateurs dont le forfait a expiré --
      // 1. Récupérer les ID des forfaits gratuits par rôle
      const [freeForfaits] = await db.query('SELECT id, type_utilisateur FROM forfaits WHERE prix_mensuel = 0');
      const freeForfaitIds = {};
      freeForfaits.forEach(f => freeForfaitIds[f.type_utilisateur] = f.id);

      // 2. Trouver les utilisateurs expirés (<= aujourd'hui) qui ont toujours un forfait payant
      const [alreadyExpired] = await db.query(`
        SELECT u.id, u.email, u.role, f.nom as forfait_nom
        FROM users u
        JOIN forfaits f ON u.forfait_id = f.id
        WHERE u.forfait_date_expiration IS NOT NULL
        AND DATE(u.forfait_date_expiration) <= DATE(?)
      ...
      console.log(`🔽 ${alreadyExpired.length} forfait(s) payant(s) expiré(s) à rétrograder.`);

      for (const user of alreadyExpired) {
        const newForfaitId = freeForfaitIds[user.role];
        if (newForfaitId) {
          await db.query(`
            UPDATE users 
            SET forfait_id = ?, 
                forfait_date_debut = NULL, 
                forfait_date_expiration = NULL,
                forfait_statut = 'actif'
            WHERE id = ?
          `, [newForfaitId, user.id]);
          console.log(`   -> Utilisateur ${user.email} (${user.role}) rétrogradé au forfait gratuit (ID: ${newForfaitId})`);
        }
      }
      // --------------------------------------------------------------------
