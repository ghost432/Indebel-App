...
exports.getMissionStats = async (req, res, next) => {
  try {
    ...
    res.json({
      success: true,
      data: {
        total: Number(hourlyStats[0]?.total || 0) + Number(fixedStats[0]?.total || 0),
        ouverts: Number(hourlyStats[0]?.ouverts || 0) + Number(fixedStats[0]?.ouverts || 0),
        en_cours: Number(hourlyStats[0]?.en_cours || 0) + Number(fixedStats[0]?.en_cours || 0),
        terminees: Number(hourlyStats[0]?.terminees || 0) + Number(fixedStats[0]?.terminees || 0),
