...
exports.createDemande = async (req, res, next) => {
  try {
    const { mission_id, mission_type, message_freelancer } = req.body;
    ...
    const [result] = await db.query(
      `INSERT INTO demandes_missions 
       (mission_id, mission_type, freelancer_id, employer_id, message_freelancer, statut, is_freelancer_job) 
       VALUES (?, ?, ?, ?, ?, 'en_attente', ?)`,
      [mission_id, mission_type, freelancer_id, mission.employer_id, message_freelancer, isFreelancerJob ? 1 : 0]
    );
