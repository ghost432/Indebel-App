...
exports.getUnreadCount = async (req, res, next) => {
  try {
    ...
    let count = 0;
    if (isAdmin) {
      let q = "SELECT COUNT(*) as count FROM support_tickets t INNER JOIN users u ON t.user_id = u.id WHERE t.statut IN ('ouvert', 'en_cours')";
      let p = [];
      if (req.user.email !== 'noreply@indebel.be') {
...
exports.getSupportStats = async (req, res, next) => {
  try {
    ...
    let baseQuery = 'FROM support_tickets t INNER JOIN users u ON t.user_id = u.id';
    let p = [];
    if (req.user.email !== 'noreply@indebel.be') {
      baseQuery += ' WHERE u.created_by = ? OR u.id = ?';
      p.push(req.user.id, req.user.id);
    }
    
    const [total] = await db.query(`SELECT COUNT(*) as total ${baseQuery}`, p);
    const [byStatus] = await db.query(`SELECT t.statut, COUNT(*) as count ${baseQuery} GROUP BY t.statut`, p);
