  ...
  try {
    ...
    if (otpRecord.user_id) {
      ...
      const [users] = await db.query(
        'SELECT id, email, role, nom, prenom, denomination FROM users WHERE id = ?',
        [otpRecord.user_id]
      );
        ...
        if (otpRecord.type === 'registration') {
          try {
            ...
            if (user.role === 'freelancer') {
              welcomeEmailConfig = emailTemplates.welcomeFreeFreelancer(
                user.prenom || '',
                user.nom || '',
                user.email
              );
            } else if (user.role === 'employer') {
              welcomeEmailConfig = emailTemplates.welcomeFreeEmployer(
